...
def calcul_utilisation_cpu_systeme(cpu, boot_time, clock_ticks_per_second):
    total_time = cpu.utime + cpu.stime # en clock ticks
    idle_time = cpu.idle[0] # en clock ticks

    total_time_sec = total_time / clock_ticks_per_second
    idle_time_sec = idle_time / clock_ticks_per_second

    stattime_sec = boot_time / clock_ticks_per_second
...
def utilisation_cpus(frequency, interval, result):
    ...
    while process_info.read_stat() != -1 and uptime_info.read_proc_uptime() != -1 and now - start < interval:
        ...
        list_cpu.append(calcul_utilisation_cpu_systeme(process_info.cpu_stats.get("cpu"), process_info.boot_time, 100))
        list_temps.append(now - start)
